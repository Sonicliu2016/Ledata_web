<template >
  <div id = "container" :class = "{'back': isShow}" >
    ...
    <!-- / / 添加关闭pdf功能-->
    <!-- <span :class = "{'close':isShow}" @click = "showPDF('../../assets/helpDoc.pdf')">开启</span> -->
    <span v-if="isShow" :class = "{'close':isShow}" @click = "closePdf">关闭</span>
...
export default {
 ...
 methods: {
  ...
  showPDF(url) {
    this.pdfUrl = url;
    this.isShow = true
    let _this = this.PDFJS.getDocument(url).then(function(pdf) {
    _this.pdfDoc = pdf
  ...
  let _this = this.pdfDoc.getPage(num).then(function(page) {
